...
export default async function BrandCategoryPage({
  ...
  return (
    <>
      ...
      {/* Shop by series (Tier-3 internal links) — only for multi-series brands */}
      {data.puffSeries.filter(s => s.count >= 4).length >= 2 && (
        <section aria-label={`${data.brand} series`} className="border-t border-gray-100 py-10">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="mb-4 text-lg font-bold text-gray-900">Shop {data.brand} by puff count</h2>
            <div className="flex flex-wrap gap-2">
              {data.puffSeries.filter(s => s.count >= 4).map(s => (
                <Link
                  key={s.puff}
                ...
                >
                  {data.brand} {s.puff.toLocaleString()} Puffs <span className="text-xs text-gray-400">{s.count}</span>
                </Link>
              ))}
            </div>
          </div>
